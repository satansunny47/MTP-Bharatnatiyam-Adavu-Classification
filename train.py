...
def train(data_type, seq_length, model, saved_model=None,
    ...
    def get_dataset(generator, batch_size):
        dataset = tf.data.Dataset.from_generator(
            lambda: generator,
            output_signature=(
                tf.TensorSpec(shape=(batch_size, seq_length, 2048), dtype=tf.float32),
                tf.TensorSpec(shape=(batch_size, class_limit), dtype=tf.float32),
            )
        )
        return dataset
    
    dataset = get_dataset(generator, batch_size)
    val_dataset = get_dataset(val_generator, batch_size)
